...
if info.get('auth_mode','api_key')!='verify':
    headers['X-API-Key']=json.loads(Path(info['systems_file']).read_text())[0]['api_key']
...
def call(payload):
    req=urllib.request.Request(info['http_url']+'/process',data=json.dumps(dict(payload=payload,payload_id=pid)).encode(),headers=headers)
